...
const TableList: React.FC = () => {
  ...
  const handleOffline = async (record: API.IdRequest) => {
    ...
    try {
      ...
      hide();
      message.error('下线失败,请重试' + error.message);
      return false;
    }
  };
  ...
  const handleRemove = async (record: API.DeleteRequest) => {
    ...
    if (!record) return true;
    try {
      await deleteInterfaceInfoUsingPost(record);
      hide();
  ...
  return (
    <PageContainer>
      <ProTable<API.RuleListItem, API.PageParams>
        ...
        request={async (params: API.InterfaceInfoVO, sort: Record<string, SortOrder>, filter: Record<string, (string | number)[] | null>) => {
